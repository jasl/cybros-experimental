...
//! Device IDs pallet benchmarking.
...
fn create_collection<T: Config<I>, I: 'static>(
	...
	assert_ok!(DeviceIds::<T, I>::force_create(
...
fn add_collection_metadata<T: Config<I>, I: 'static>() -> (T::AccountId, AccountIdLookupOf<T>) {
	...
	let caller_lookup = T::Lookup::unlookup(caller.clone());
	assert_ok!(DeviceIds::<T, I>::set_collection_metadata(
...
fn mint_item<T: Config<I>, I: 'static>(
	...
	if item_exists {
		...
		assert_ok!(DeviceIds::<T, I>::force_mint(
			SystemOrigin::Signed(caller.clone()).into(),
			collection,
			item,
			caller_lookup.clone(),
...
fn lock_item<T: Config<I>, I: 'static>(
	...
	let caller_lookup = T::Lookup::unlookup(caller.clone());
	let item = T::Helper::item(index);
	assert_ok!(DeviceIds::<T, I>::lock_item_transfer(
		SystemOrigin::Signed(caller.clone()).into(),
		T::Helper::collection(0),
		item,
	));
...
fn add_item_metadata<T: Config<I>, I: 'static>(
	...
	let caller_lookup = T::Lookup::unlookup(caller.clone());
	assert_ok!(DeviceIds::<T, I>::set_metadata(
		SystemOrigin::Signed(caller.clone()).into(),
		T::Helper::collection(0),
		item,
		vec![0; T::StringLimit::get() as usize].try_into().unwrap(),
...
fn add_item_attribute<T: Config<I>, I: 'static>(
	...
	assert_ok!(DeviceIds::<T, I>::set_attribute(
		SystemOrigin::Signed(caller.clone()).into(),
		T::Helper::collection(0),
		Some(item),
...
benchmarks_instance_pallet! {
	...
	redeposit {
		...
		DeviceIds::<T, I>::force_collection_config(
			SystemOrigin::Root.into(),
			collection,
	...
	unlock_item_transfer {
		let (collection, caller, _) = create_collection::<T, I>();
		let (item, ..) = mint_item::<T, I>(0);
		DeviceIds::<T, I>::lock_item_transfer(
			SystemOrigin::Signed(caller.clone()).into(),
			collection,
			item,
		)?;
	...
	transfer_ownership {
		...
		DeviceIds::<T, I>::set_accept_ownership(origin, Some(collection))?;
	...
	cancel_item_attributes_approval {
		...
		let target: T::AccountId = account("target", 0, SEED);
		let target_lookup = T::Lookup::unlookup(target.clone());
		DeviceIds::<T, I>::approve_item_attributes(
			SystemOrigin::Signed(caller.clone()).into(),
			collection,
			item,
			target_lookup.clone(),
	...
	cancel_approval {
		...
		let deadline = BlockNumberFor::<T>::max_value();
		DeviceIds::<T, I>::approve_transfer(origin, collection, item, delegate_lookup.clone(), Some(deadline))?;
	...
	buy_item {
		...
		DeviceIds::<T, I>::set_price(origin, collection, item, Some(price), Some(buyer_lookup))?;
	...
	impl_benchmark_test_suite!(DeviceIds, mock::new_test_ext(), crate::mock::Test);
}
