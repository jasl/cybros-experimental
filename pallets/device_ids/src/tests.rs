...
//! Tests for the Device IDs pallet.
...
fn basic_minting_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::mint(RuntimeOrigin::signed(account(2)), 1, 69, account(1), None));
...
fn lifecycle_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::burn(RuntimeOrigin::signed(account(10)), 0, 42));
		assert_ok!(DeviceIds::burn(RuntimeOrigin::signed(account(20)), 0, 69));
...
fn destroy_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::mint(RuntimeOrigin::signed(account(1)), 0, 42, account(2), None));
		assert_noop!(
			DeviceIds::destroy(
				RuntimeOrigin::signed(account(1)),
				0,
...
fn mint_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::update_mint_settings(
			RuntimeOrigin::signed(account(1)),
			0,
			MintSettings {
				start_block: Some(2),
		...
		assert_noop!(
			DeviceIds::mint(RuntimeOrigin::signed(account(2)), 0, 43, account(1), None),
		...
		Balances::make_free_balance_be(&account(2), 100);
		assert_noop!(
			DeviceIds::mint(RuntimeOrigin::signed(account(2)), 0, 43, account(2), None,),
		...
		assert_noop!(
			DeviceIds::mint(RuntimeOrigin::signed(account(3)), 1, 42, account(3), None),
			Error::<Test>::WitnessRequired
		);
		assert_noop!(
			DeviceIds::mint(RuntimeOrigin::signed(account(2)), 1, 42, account(2), None),
			Error::<Test>::WitnessRequired
		);
		assert_noop!(
			DeviceIds::mint(
		...
		assert!(events().contains(&Event::<Test>::PalletAttributeSet {
			...
			value: DeviceIds::construct_attribute_value(vec![]).unwrap(),
		}));
...
fn transfer_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::transfer(RuntimeOrigin::signed(account(2)), 0, 42, account(3)));
		assert_eq!(items(), vec![(account(3), 0, 42)]);
		assert_noop!(
			DeviceIds::transfer(RuntimeOrigin::signed(account(2)), 0, 42, account(4)),
			Error::<Test>::NoPermission
		);

		assert_ok!(DeviceIds::approve_transfer(
		...
		assert_ok!(DeviceIds::transfer(RuntimeOrigin::signed(account(2)), 0, 42, account(4)));
		...
		assert_noop!(
			DeviceIds::transfer(RuntimeOrigin::signed(account(1)), collection_id, 42, account(3)),
...
fn locking_transfer_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::lock_item_transfer(RuntimeOrigin::signed(account(1)), 0, 42));
		assert_noop!(
			DeviceIds::transfer(RuntimeOrigin::signed(account(1)), 0, 42, account(2)),
			Error::<Test>::ItemLocked
		);

		assert_ok!(DeviceIds::unlock_item_transfer(RuntimeOrigin::signed(account(1)), 0, 42));
		...
		assert_ok!(DeviceIds::force_collection_config(
			RuntimeOrigin::root(),
			0,
			collection_config_with_all_settings_enabled(),
		));
		assert_ok!(DeviceIds::transfer(RuntimeOrigin::signed(account(1)), 0, 42, account(2)));
	});
}
...
fn origin_guards_should_work() {
	new_test_ext().execute_with(|| {
		...
		Balances::make_free_balance_be(&account(2), 100);
		assert_ok!(DeviceIds::set_accept_ownership(RuntimeOrigin::signed(account(2)), Some(0)));
		assert_noop!(
			DeviceIds::transfer_ownership(RuntimeOrigin::signed(account(2)), 0, account(2)),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			DeviceIds::set_team(
		...
		assert_noop!(
			DeviceIds::lock_item_transfer(RuntimeOrigin::signed(account(2)), 0, 42),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			DeviceIds::unlock_item_transfer(RuntimeOrigin::signed(account(2)), 0, 42),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			DeviceIds::mint(RuntimeOrigin::signed(account(2)), 0, 69, account(2), None),
			Error::<Test>::NoPermission
		);
		assert_ok!(DeviceIds::mint(RuntimeOrigin::signed(account(1)), 0, 43, account(2), None));
		assert_noop!(
			DeviceIds::burn(RuntimeOrigin::signed(account(1)), 0, 43),
			Error::<Test>::NoPermission
		);
		let w = DeviceIds::get_destroy_witness(&0).unwrap();
		assert_noop!(
			DeviceIds::destroy(RuntimeOrigin::signed(account(2)), 0, w),
			Error::<Test>::NoPermission
		);
	});
}
...
fn transfer_owner_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(collections(), vec![(account(1), 0)]);
		assert_noop!(
			DeviceIds::transfer_ownership(RuntimeOrigin::signed(account(1)), 0, account(2)),
		...
		assert_ok!(DeviceIds::transfer_ownership(RuntimeOrigin::signed(account(1)), 0, account(2)));
		...
		assert_ok!(DeviceIds::set_accept_ownership(RuntimeOrigin::signed(account(1)), Some(0)));
		assert_noop!(
			DeviceIds::transfer_ownership(RuntimeOrigin::signed(account(1)), 0, account(1)),
			Error::<Test>::NoPermission
		);
		...
		assert_noop!(
			DeviceIds::transfer_ownership(RuntimeOrigin::signed(account(3)), 0, account(2)),
			Error::<Test>::Unaccepted
		);
	});
}
...
fn set_team_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::mint(RuntimeOrigin::signed(account(2)), 0, 42, account(2), None));

		// admin can't transfer/burn items he doesn't own
		assert_noop!(
			DeviceIds::transfer(RuntimeOrigin::signed(account(3)), 0, 42, account(3)),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			DeviceIds::burn(RuntimeOrigin::signed(account(3)), 0, 42),
			Error::<Test>::NoPermission
		);

		assert_ok!(DeviceIds::lock_item_transfer(RuntimeOrigin::signed(account(4)), 0, 42));
		assert_ok!(DeviceIds::unlock_item_transfer(RuntimeOrigin::signed(account(4)), 0, 42));
		...
		assert_noop!(
			DeviceIds::lock_item_transfer(RuntimeOrigin::signed(account(4)), 0, 42),
			Error::<Test>::NoPermission
		);

		// set all the roles to None
		assert_ok!(DeviceIds::set_team(RuntimeOrigin::signed(account(1)), 0, None, None, None,));
...
fn set_collection_metadata_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::set_collection_metadata(RuntimeOrigin::root(), 0, bvec![0u8; 18]));
		...
		assert_noop!(
			DeviceIds::clear_collection_metadata(RuntimeOrigin::signed(account(1)), 0),
			Error::<Test>::LockedCollectionMetadata
		);

		// Clear Metadata
		assert_ok!(DeviceIds::set_collection_metadata(RuntimeOrigin::root(), 0, bvec![0u8; 15]));
		assert_noop!(
			DeviceIds::clear_collection_metadata(RuntimeOrigin::signed(account(2)), 0),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			DeviceIds::clear_collection_metadata(RuntimeOrigin::signed(account(1)), 1),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			DeviceIds::clear_collection_metadata(RuntimeOrigin::signed(account(1)), 0),
		...
		assert_ok!(DeviceIds::clear_collection_metadata(RuntimeOrigin::root(), 0));
		assert!(!CollectionMetadataOf::<Test>::contains_key(0));
	});
}
...
fn set_item_metadata_should_work() {
	new_test_ext().execute_with(|| {
		...
		// Cannot add metadata to unowned item
		assert_noop!(
			DeviceIds::set_metadata(RuntimeOrigin::signed(account(2)), 0, 42, bvec![0u8; 20]),
			Error::<Test>::NoPermission,
		);
		...
		// Cannot over-reserve
		assert_noop!(
			DeviceIds::set_metadata(RuntimeOrigin::signed(account(1)), 0, 42, bvec![0u8; 40]),
			BalancesError::<Test, _>::InsufficientBalance,
		);
		...
		assert_noop!(
			DeviceIds::set_metadata(RuntimeOrigin::signed(account(1)), 0, 42, bvec![0u8; 15]),
			Error::<Test, _>::LockedItemMetadata,
		);
		assert_noop!(
			DeviceIds::clear_metadata(RuntimeOrigin::signed(account(1)), 0, 42),
			Error::<Test>::LockedItemMetadata,
		);

		// Clear Metadata
		assert_ok!(DeviceIds::set_metadata(RuntimeOrigin::root(), 0, 42, bvec![0u8; 15]));
		assert_noop!(
			DeviceIds::clear_metadata(RuntimeOrigin::signed(account(2)), 0, 42),
			Error::<Test>::NoPermission,
		);
		assert_noop!(
			DeviceIds::clear_metadata(RuntimeOrigin::signed(account(1)), 1, 42),
			Error::<Test>::NoPermission,
		);
		assert_ok!(DeviceIds::clear_metadata(RuntimeOrigin::root(), 0, 42));
		assert!(!ItemMetadataOf::<Test>::contains_key(0, 42));
	});
}
...
fn set_collection_owner_attributes_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::mint(RuntimeOrigin::signed(account(1)), 0, 0, account(1), None));
		...
		assert_ok!(DeviceIds::burn(RuntimeOrigin::root(), 0, 0));
		let w = DeviceIds::get_destroy_witness(&0).unwrap();
		assert_ok!(DeviceIds::destroy(RuntimeOrigin::signed(account(1)), 0, w));
		assert_eq!(attributes(0), vec![]);
		assert_eq!(Balances::reserved_balance(account(1)), 0);
...
fn set_collection_system_attributes_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(<DeviceIds as Mutate<AccountIdOf<Test>, ItemConfig>>::set_collection_attribute(
			&collection_id,
			&attribute_key,
			&attribute_value
		));
		...
		assert_ok!(
			<DeviceIds as Mutate<AccountIdOf<Test>, ItemConfig>>::set_typed_collection_attribute(
				&collection_id,
				&typed_attribute_key,
		...
		assert_ok!(DeviceIds::burn(RuntimeOrigin::root(), collection_id, 0));
		let w = DeviceIds::get_destroy_witness(&0).unwrap();
		assert_ok!(DeviceIds::destroy(RuntimeOrigin::signed(account(1)), collection_id, w));
...
fn set_item_owner_attributes_should_work() {
	new_test_ext().execute_with(|| {
		...
		// transfer item
		assert_ok!(DeviceIds::transfer(RuntimeOrigin::signed(account(2)), 0, 0, account(3)));
		...
		assert_ok!(DeviceIds::burn(RuntimeOrigin::signed(account(3)), 0, 0));
		assert_eq!(
			attributes(0),
			vec![
...
fn set_external_account_attributes_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::approve_item_attributes(
			RuntimeOrigin::signed(account(1)),
			0,
			0,
			account(2)
		...
		assert_ok!(DeviceIds::cancel_item_attributes_approval(
			RuntimeOrigin::signed(account(1)),
			0,
			0,
			account(2),
...
fn validate_deposit_required_setting() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(
			<DeviceIds as Mutate<<Test as SystemConfig>::AccountId, ItemConfig>>::clear_attribute(
				&0,
				&0,
				&[3],
			)
...
fn set_attribute_should_respect_lock() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::mint(RuntimeOrigin::signed(account(1)), 0, 0, account(1), None));
		assert_ok!(DeviceIds::mint(RuntimeOrigin::signed(account(1)), 0, 1, account(1), None));
...
fn preserve_config_for_frozen_items() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::burn(RuntimeOrigin::signed(account(1)), 0, 1));
		...
		assert_ok!(DeviceIds::burn(RuntimeOrigin::signed(account(1)), 0, 0));
		let config = ItemConfigOf::<Test>::get(0, 0).unwrap();
		assert_eq!(config, expect_config);

		// can't mint with the different config
		assert_noop!(
			DeviceIds::force_mint(
				RuntimeOrigin::signed(account(1)),
				0,
				0,
				account(2),
...
fn force_update_collection_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::set_metadata(RuntimeOrigin::signed(account(1)), 0, 42, bvec![0; 20]));
		assert_ok!(DeviceIds::set_metadata(RuntimeOrigin::signed(account(1)), 0, 69, bvec![0; 20]));
		...
		assert_ok!(DeviceIds::mint(RuntimeOrigin::signed(account(1)), 0, 142, account(1), None));
		assert_ok!(DeviceIds::force_mint(
			RuntimeOrigin::signed(account(1)),
			0,
			169,
		...
		assert_ok!(DeviceIds::set_metadata(RuntimeOrigin::signed(account(5)), 0, 42, bvec![0; 20]));
		assert_eq!(Balances::reserved_balance(account(5)), 42);

		assert_ok!(DeviceIds::set_metadata(RuntimeOrigin::signed(account(5)), 0, 69, bvec![0; 20]));
...
fn burn_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceIds::burn(RuntimeOrigin::signed(account(5)), 0, 42),
		...
		assert_noop!(
			DeviceIds::burn(RuntimeOrigin::signed(account(0)), 0, 42),
			Error::<Test>::NoPermission
		);

		assert_ok!(DeviceIds::burn(RuntimeOrigin::signed(account(5)), 0, 42));
		assert_ok!(DeviceIds::burn(RuntimeOrigin::signed(account(5)), 0, 69));
		assert_eq!(Balances::reserved_balance(account(1)), 0);
	});
}
...
fn approval_lifecycle_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::transfer(RuntimeOrigin::signed(account(3)), 0, 42, account(4)));
		assert_noop!(
			DeviceIds::transfer(RuntimeOrigin::signed(account(3)), 0, 42, account(3)),
			Error::<Test>::NoPermission
		);
		...
		assert_ok!(DeviceIds::transfer(RuntimeOrigin::signed(account(2)), 0, 42, account(2)));
...
fn cancel_approval_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceIds::cancel_approval(RuntimeOrigin::signed(account(2)), 1, 42, account(3)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			DeviceIds::cancel_approval(RuntimeOrigin::signed(account(2)), 0, 43, account(3)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			DeviceIds::cancel_approval(RuntimeOrigin::signed(account(3)), 0, 42, account(3)),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			DeviceIds::cancel_approval(RuntimeOrigin::signed(account(2)), 0, 42, account(4)),
		...
		assert_noop!(
			DeviceIds::cancel_approval(RuntimeOrigin::signed(account(5)), 0, 42, account(3)),
			Error::<Test>::NoPermission
		);
...
fn approving_multiple_accounts_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::transfer(RuntimeOrigin::signed(account(4)), 0, 42, account(6)));
		assert_noop!(
			DeviceIds::transfer(RuntimeOrigin::signed(account(3)), 0, 42, account(7)),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			DeviceIds::transfer(RuntimeOrigin::signed(account(5)), 0, 42, account(8)),
			Error::<Test>::NoPermission
		);
	});
}
...
fn approval_deadline_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceIds::transfer(RuntimeOrigin::signed(account(3)), 0, 42, account(4)),
		...
		assert_ok!(DeviceIds::transfer(RuntimeOrigin::signed(account(6)), 0, 42, account(5)));
	});
}
...
fn cancel_approval_works_with_admin() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceIds::cancel_approval(RuntimeOrigin::signed(account(2)), 1, 42, account(1)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			DeviceIds::cancel_approval(RuntimeOrigin::signed(account(2)), 0, 43, account(1)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			DeviceIds::cancel_approval(RuntimeOrigin::signed(account(2)), 0, 42, account(4)),
...
fn cancel_approval_works_with_force() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceIds::cancel_approval(RuntimeOrigin::root(), 1, 42, account(1)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			DeviceIds::cancel_approval(RuntimeOrigin::root(), 0, 43, account(1)),
			Error::<Test>::UnknownItem
		);
		assert_noop!(
			DeviceIds::cancel_approval(RuntimeOrigin::root(), 0, 42, account(4)),
			Error::<Test>::NotDelegate
		);

		assert_ok!(DeviceIds::cancel_approval(RuntimeOrigin::root(), 0, 42, account(3)));
		assert_noop!(
			DeviceIds::cancel_approval(RuntimeOrigin::root(), 0, 42, account(1)),
			Error::<Test>::NotDelegate
		);
	});
}
...
fn clear_all_transfer_approvals_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceIds::clear_all_transfer_approvals(RuntimeOrigin::signed(account(3)), 0, 42),
			Error::<Test>::NoPermission
		);
		...
		assert_noop!(
			DeviceIds::transfer(RuntimeOrigin::signed(account(3)), 0, 42, account(5)),
			Error::<Test>::NoPermission
		);
		assert_noop!(
			DeviceIds::transfer(RuntimeOrigin::signed(account(4)), 0, 42, account(5)),
			Error::<Test>::NoPermission
		);
	});
}
...
fn max_supply_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::set_collection_max_supply(
		...
		assert_noop!(
			DeviceIds::set_collection_max_supply(
				RuntimeOrigin::signed(user_id.clone()),
				collection_id,
				max_supply + 2
			),
...
fn buy_item_should_work() {
	new_test_ext().execute_with(|| {
		...
		// can't buy for less
		assert_noop!(
			DeviceIds::buy_item(RuntimeOrigin::signed(user_2.clone()), collection_id, item_1, 1),
			Error::<Test>::BidTooLow
		);

		// pass the higher price to validate it will still deduct correctly
		assert_ok!(DeviceIds::buy_item(
		...
		assert_noop!(
			DeviceIds::buy_item(RuntimeOrigin::signed(user_1.clone()), collection_id, item_2, price_2),
			Error::<Test>::NoPermission
		);

		// can't buy when the item is listed for a specific buyer
		assert_noop!(
			DeviceIds::buy_item(RuntimeOrigin::signed(user_2.clone()), collection_id, item_2, price_2),
			Error::<Test>::NoPermission
		);
		...
		assert_noop!(
			DeviceIds::buy_item(RuntimeOrigin::signed(user_2.clone()), collection_id, item_3, price_2),
		...
		// ensure we can't buy an item when the collection or an item are frozen
		{
			...
			let buy_item_call = mock::RuntimeCall::DeviceIds(Call::<Test>::buy_item {
				collection: collection_id,
				item: item_3,
				bid_price: price_1,
			});
			...
			// lock the transfer
			assert_ok!(DeviceIds::lock_item_transfer(
				RuntimeOrigin::signed(user_1.clone()),
				collection_id,
				item_3,
			));
...
fn various_collection_settings() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::force_create(RuntimeOrigin::root(), account(1), config));
...
fn pallet_level_feature_flags_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceIds::buy_item(RuntimeOrigin::signed(user_id.clone()), collection_id, item_id, 1),
			Error::<Test>::MethodDisabled
		);
...
fn add_remove_item_attributes_approval_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			DeviceIds::approve_item_attributes(
				RuntimeOrigin::signed(user_1.clone()),
				collection_id,
				item_id,
				user_4,
...
fn pre_signed_mints_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::mint_pre_signed(
			RuntimeOrigin::signed(user_2.clone()),
		...
		assert_ok!(DeviceIds::burn(RuntimeOrigin::signed(user_2.clone()), 0, 0));
...
fn clear_collection_metadata_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(DeviceIds::clear_collection_metadata(RuntimeOrigin::signed(account(1)), 0));
		assert_eq!(Collection::<Test>::get(0).unwrap().owner_deposit, 2);
		assert_eq!(Balances::reserved_balance(&account(1)), 12);
